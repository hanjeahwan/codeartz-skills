...
**实际结果：** 证据达到 L3 且方案关卡通过后，写入 `.codeartz/<topic>/target-boundary.md`；否则只输出证据或合同缺口。待裁决、冲突和阻塞开工的未知分区处理完成，风险已经展示，并且用户同意采用当前方案后，才生成独立的 `context-handoff.md`。
...
| [`target-boundary`](skills/target-boundary/)                   | 需求资料与现有系统事实混杂                   | 关卡通过后写方案；采用方案后生成交接文件 |
